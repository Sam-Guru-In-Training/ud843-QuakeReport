...
/**
 * Created by Anand Prabhu on 25/07/16.
...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            // Convert response String into JSONObject
            JSONObject rootObject = new JSONObject(SAMPLE_JSON_RESPONSE);

            // Extract the 'features' JSONArray from root JSONObject
            JSONArray featuresJsonArray = rootObject.optJSONArray("features");

            // Loop through the 'features' JSONArray
            for (int i = 0; i < featuresJsonArray.length(); i++) {
                JSONObject earthquakesJsonObject = featuresJsonArray.getJSONObject(i);

                JSONObject propertiesJsonObject = earthquakesJsonObject.getJSONObject("properties");

                double magnitude = propertiesJsonObject.getDouble("mag");
                String location = propertiesJsonObject.getString("place");

                // Convert the time to human readable format using SimpleDateFormat
                long time = Long.valueOf(propertiesJsonObject.getString("time"));
